...
-- COMMENTED OUT: This policy was causing infinite recursion
...
-- Create separate policies for each operation instead
CREATE POLICY "Memorial owners can select"
  ON memorials FOR SELECT
  TO public
  USING (user_id = auth.uid());

CREATE POLICY "Memorial owners can insert"
  ON memorials FOR INSERT
  TO public
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Memorial owners can update"
  ON memorials FOR UPDATE
  TO public
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Memorial owners can delete"
...
-- COMMENTED OUT: This policy may be causing infinite recursion
